...
describe User do
  ...
  it "allows users to have the same name" do
    FactoryGirl.create(:user, name: "steve")
    FactoryGirl.build(:user, name: "steve").should be_valid
  end
  ...
  it "has a name larger than or equal to 1 character" do
    FactoryGirl.build(:user, name: "D").should be_valid
  end

  it "has a name less than 25 characters" do
    long_name = "a" * 26
    FactoryGirl.build(:user, name: long_name).should_not be_valid
  end

  it "responds to name attribute" do
    FactoryGirl.create(:user).should respond_to :name
  end
